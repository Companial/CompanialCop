...
{
    public delegate bool Filter(IGrouping<int, Rule0027AnalyzeTransferField.Field> fieldGroup);

    [DiagnosticAnalyzer]
    public class Rule0027AnalyzeTransferField : DiagnosticAnalyzer
    {
        ...
        {
            context.RegisterSyntaxNodeAction(new Action<SyntaxNodeAnalysisContext>(this.AnalyzeTableExtension), SyntaxKind.TableExtensionObject);
        ...
        private async void AnalyzeTransferField(OperationAnalysisContext ctx)
        {
        ...
        private void ReportFieldDiagnostics(OperationAnalysisContext ctx, Table table, List<IGrouping<int, Field>> fieldGroups)
        {
            foreach (IGrouping<int, Field> fieldGroup in fieldGroups)
            {
                ...
                foreach (Field fieldGroupValue in fieldGroupValues)
                {
                    ...
                    ctx.ReportDiagnostic(Diagnostic.Create(DiagnosticDescriptors.Rule0027AnalyzeTableExtension, field.Location, fieldGroupValue.Table.Name));
                }
            }
        }
        ...
        public class Table
        {
            ...
            public void PopulateFields(IApplicationObjectTypeSymbol table)
            {
                ...
                foreach (var field in collection)
                {
                    ...
                    string objtype = (string)typeprop.GetValue(field).ToString();
